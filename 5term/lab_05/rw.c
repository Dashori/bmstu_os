...
int main(void)
{
    ...
    key_t shmkey = ftok("pcfile", 3); // преобразовывает имя файла и идентификатор проекта в ключ для системных вызовов  
    ...
    int shmid = shmget(shmkey, sizeof(int), perms | IPC_CREAT); // создаем сегмент
    ...
    int *addr = shmat(shmid, NULL, 0); // подключаем сегмент к адр пространству 
    ...
    if (shmdt((void*)addr) == -1 || shmctl(shmid, IPC_RMID, NULL) == -1 || semctl(sem_id, IPC_RMID, 0) == -1)
    {
        perror("error\n");
        exit(1);
    }
...
int reader_run(int *const addr, const int sem_id, const int rdid)
{
    ...
    for (size_t i = 0; i < ITER_CNT; i++)
    {
        ...
        printf(" \e[1;31mReader #%d \tread: \t%d \e[0m \n", rdid + 1, *addr);
...
int writer_run(int *const addr, const int sem_id, const int wrid)
{
    for (size_t i = 0; i < ITER_CNT; i++)
    {
        ...
        printf(" \e[1;34mWriter #%d \twrite: %d\e[0m\n", wrid + 1, ++(*addr));
